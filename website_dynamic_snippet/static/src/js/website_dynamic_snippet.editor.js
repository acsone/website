odoo.define(
    "website_dynamic_snippet.website_dynamic_snippet_editor",
    function (require) {
        "use strict";
        ...
        var _t = core._t;
        ajax.loadXML(
            "/website_dynamic_snippet/static/src/xml/website_dynamic_snippet.editor.xml",
            core.qweb
        );

        var WebsiteDynamicFinderSettingsDialog = Dialog.extend({
            template:
                "web_editor.dialog.editor_dialog_website_dynamic_finder_related_settings",
            ...
            save: function () {
                ...
                self.$target
                    .closest("div[data-oe-model='ir.ui.view']")
                    .addClass("o_dirty");
            ...
            bind_data: function () {
                var self = this;
                self.$favorite_chk.prop(
                    "checked",
                    self.$target.attr("data-favorite") === "true"
                );
        ...
        s_options.registry.website_dynamic_finder_related_snippet =
            s_options.Class.extend({
                on_prompt: function () {
                    var self = this;
                    return new WebsiteDynamicFinderSettingsDialog(
                        self.$target,
                        {}
                    ).open();
                },
                ...
                start: function () {
                    this.$el
                        .find(".js_website_dynamic_finder_related_settings")
                        .on("click", _.bind(this.on_prompt, this));
                    this._super();
                },
                clean_for_save: function () {
                    this.$target
                        .find(".parametricTemplate")
                        .empty()
                        .append(
                            jQuery("<t />")
                                ...
                                .append(
                                    ...
                                    jQuery("<t />")
                                        .attr(
                                            "t-value",
                                            "'" +
                                                this.$target.attr("data-all-links") +
                                                "'"
                                        )
                                    ...
                                    jQuery("<t />")
                                        .attr(
                                            "t-value",
                                            this.$target.attr("data-favorite") ===
                                                "true"
                                                ? "True"
                                                : "False"
                                        )
                                    ...
                                    jQuery("<t />")
                                        .attr(
                                            "t-value",
                                            this.$target.attr("data-limit") || "None"
                                        )
                                        .attr("t-set", "limit")
                                        .attr("t-ignore-branding", "1"),
                                    jQuery("<t />")
                                        .attr(
                                            "t-value",
                                            "'" +
                                                this.$target.attr("data-model-name") +
                                                "'"
                                        )
                                    ...
                                    jQuery("<t />")
                                        .attr(
                                            "t-value",
                                            "'" +
                                                this.$target.attr("data-empty-msg") +
                                                "'"
                                        )
